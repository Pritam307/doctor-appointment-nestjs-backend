  ...
  async getAvailableSlots(doctorId: string, date: string) {
    try {
      ...
      const existingAppointments = await this.appointmentRepo
        .createQueryBuilder('appointment')
        .where('appointment.doctorId = :doctorId', { doctorId })
        .andWhere('appointment.startTime >= :dayStart', {
          dayStart: dayStart.toISOString(),
        })
        .andWhere('appointment.endTime <= :dayEnd', {
          dayEnd: dayEnd.toISOString(),
        })
        .getMany();

      console.log(
        `Found ${existingAppointments.length} existing appointments for this date:`,
      );
      existingAppointments.forEach((apt) => {
        console.log(
          `- ${apt.startTime} to ${apt.endTime} for ${apt.patientName}`,
        );
      });
